...
// return 0 jika berhasil, jika gagal return -1
...
void *producer(void *param) {
    ...
    while (1) {
        ...
        item = rand();
        if (insert_item(item)) {
            fprintf(stderr, "Error: gagal memproduksi item.\n");
        } else {
            printf("Producer %i memproduksi %d.\n", *(int *)param, item);
        }
    }
}
...
void *consumer(void *param) {
    ...
    while (1) {
        ...
        sleep(rand() % MAX_SLEEP_TIME + 1);
        if (remove_item(&item)) {
            fprintf(stderr, "Error: gagal mengkonsumsi item.\n");
        } else {
            printf("Consumer %i mengkonsumsi %d.\n", *(int *)param, item);
        }
    }
}
